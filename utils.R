...
# 
# 
# function(data, QC.index, first_batch, auto_merge = T){
#   QC_data = data[,QC.index]
#   QC_batches = first_batch[QC.index]
#   if(!auto_merge){
#     batches = matrix(first_batch,nrow=nrow(data),ncol=ncol(data),byrow = T)
#     return(batches)
#   }else{
#     batches = matrix(NA,nrow=nrow(data),ncol=ncol(data),byrow = T)
#     for(i in 1:nrow(QC_data)){
#       x = QC_data[i,]
#       p.values = diag(pairwise.wilcox.test(x,QC_batches,p.adjust.method='none')$p.value)
# 
#       if(length(which(p.values<0.05))==0){
#         batches[i,] = first_batch
#       }else if(length(which(p.values<0.05))==1){
#         index = c(which(p.values<0.05),which(p.values<0.05)+1)
#         inv_index = c(1:length(unique(first_batch)))[!c(1:length(unique(first_batch)))%in%index]
#         times = rep(NA,3)
#         times_temp = table(first_batch)[inv_index]
#         times[which(p.values<0.05)] = sum(table(first_batch)[index])
#         times[is.na(times)] = times_temp
#         batches[i,] = rep(LETTERS[1:3],times)
#       }else if(length(which(p.values<0.05))==2){
#         index = list(c(which(p.values<0.05)[1],which(p.values<0.05)[1]+1),
#                      c(which(p.values<0.05)[2],which(p.values<0.05)[2]+1))
#         index = unique(unlist(index))
#         inv_index = c(1:length(unique(first_batch)))[!c(1:length(unique(first_batch)))%in%unlist(index)]
#         times = rep(NA,3)
#         # times[which(p.values<0.05)] = sapply(index,function(x){
#         #   sum(table(first_batch)[x])
#         # })
#         times[which(p.values<0.05)] = sum(table(first_batch)[index])
#         times[is.na(times)] = table(first_batch)[inv_index]
#         batches[i,] = rep(LETTERS[1:3],times)
#       }else{
#         batches[i,] = rep("A",length(batches[i,]))
#       }
#     }
#       
#       
#   }
#   time.diff = diff(p[["TimeStamp"]])
#   batch.index = which(time.diff%in%sort(time.diff,decreasing=T)[1:(4-1)])
#   
#   if(auto.merge.batch){
#     for(i in 1:nrow(f)){
#       test=pairwise.wilcox.test(as.numeric(e[i,QC.index]), batch[QC.index],p.adjust.method = p.adjust.method)
#       test.p = diag(test$p.value)
#       batch.opt = LETTERS[1:(length(which(p.values>0.05))+1)]
#       batches[,i] = rep(batch.opt,times = diff(c(0,batch.index[which(p.values<0.05)],nrow(p))))
#       
#     }
#   }else{
#     batches = matrix(rep(batch,ncol(batches)),ncol = ncol(batches))
